    ...
    if reparos_criticos:
        for reparo in reparos_criticos:
            ...
            if gestor_b2b is None:
                logging.warning(f"Nenhum gestor B2B encontrado para a UF: {uf}")
                continue

            chat_id = gestor_b2b['chatid_cadastro']
            if not chat_id:
                logging.warning(f"Usuário ainda não possui chat_id para UF:{uf} e regional: {regional}")
                continue
            ...
            if reparo['status'] not in ('Fechado', 'Blacklist') and reparo['escalonado'] == 'COBRANCA':
                ...
                if estado_circuito in ['aguardando_previsao']:
                    logging.info(f"Circuito {circuito} já está aguardando resposta. Não será processada nova cobrança.")
                    continue



                # Verifica se o estado é "cobrança_encerrada" e se a próxima cobrança é válida
                if estado_resposta_usuario.get(f"{chat_id}_{circuito}_{numero}") == "cobrança_encerrada":
                    if proxima_cobranca and proxima_cobranca <= data_atual:
                        ...
                        break
                logging.info(f"[processar_reparos]estado_resposta_usuario.values(): {estado_resposta_usuario.values()}")

                # Verifica se há cobrança pendente para esse circuito específico
                if estado_resposta_usuario.get(f"{chat_id}_{circuito}_{numero}") == 'esperando_opcao':
                    ...
                    if proxima_cobranca is None or proxima_cobranca <= data_atual:
                        try:
                            ...
                            if nv_escalacao == 2:
                                ...
                                proxima_cobranca = data_atual + timedelta(minutes=15)
            ...
            if len(fila_circuitos) > 0:
                ...
                logging.info(f"[processar_reparos] Processando cobrança para circuito: {circuito_atual}, chat_id: {chat_id}")

                estado_circuito_atual = estado_resposta_usuario.get(f"{chat_id}_{circuito_atual}_{numero}")
                logging.info(f"[processar_reparos] Estado do circuito atual {circuito_atual}: {estado_circuito_atual}")

                # Verifica se já há cobrança em andamento para este circuito
                if estado_circuito_atual != 'esperando_opcao':
                    # Processar o próximo circuito se houver outro aguardando
                    if any(estado == 'esperando_opcao' for key, estado in estado_resposta_usuario.items() if key.startswith(f"{chat_id}_")):
                        logging.info(f"[processar_reparos] Já existe uma cobrança ativa para o chat_id {chat_id}.")
                    else:
                        try:
                            estado_resposta_usuario[f"{chat_id}_{circuito_atual}_{numero}"] = 'esperando_opcao'
                            logging.info(f"[processar_reparos] Marcado como 'esperando_opcao' para circuito {circuito_atual} e chat_id {chat_id}.")
                            ...
                            logging.info(f"[processar_reparos] Próxima cobrança agendada para {proxima_cobranca} para o circuito {circuito_atual}.")
                            ...
                            logging.info(f"[processar_reparos] Cobrança enviada para o circuito {circuito_atual}.")

                        except telebot.apihelper.ApiTelegramException as e:
                            logging.error(f"Erro ao enviar mensagem para {chat_id}: {e}")
                else:
                    logging.info(f"[processar_reparos] Circuito {circuito_atual} já está em cobrança. Verificando respostas.")
                    # Resetar o estado após receber a resposta
                    estado_resposta_usuario[f"{chat_id}_{circuito_atual}_{numero}"] = None
                    logging.info(f"[processar_reparos] Estado resetado para circuito {circuito_atual}.")
        ...
        for reparo in reparos_fechados:
            ...
            if reparo['status'] in ('Fechado', 'Blacklist') and escalonado == 'COBRANCA':
                ...
                for chat_id, hierarquia, gestor, uf_usuario, regional_usuario in usuarios:
                    ...
                    # Condições específicas para Diretores em diferentes níveis de escala
                ...
                estado_chave = f"{chat_id}_{circuito}_{numero}"
                if estado_chave in estado_resposta_usuario:
                    del estado_resposta_usuario[estado_chave]
                    logging.info(f"[processar_reparos_fechados] Circuito {circuito} removido da fila e estado de resposta limpo.")
...
def enviar_opcoes_escalacao(chat_id, circuito, numero):
    global estado_resposta_usuario, circuitos_usuario
    ...
    logging.info(f"[enviar_opcoes_escalacao] estado_resposta_usuario[{chat_id}_{circuito}_{numero}]: {estado_resposta_usuario.get(f'{chat_id}_{circuito}_{numero}', 'None')}")

    # Armazena o circuito associado ao chat_id
    circuitos_usuario[chat_id] = {'circuito': circuito, 'numero': numero}
...
def tratar_resposta_escolha(message):
    ...
    circuito_info = circuitos_usuario.get(user_id, {})
    circuito = circuito_info.get('circuito')
    numero = circuito_info.get('numero')

    estado_chave = f"{user_id}_{circuito}_{numero}"
    if estado_chave not in estado_resposta_usuario or estado_resposta_usuario[estado_chave] != 'esperando_opcao':
    ...
    if circuito:
        ...
        if escolha == '1':
            estado_resposta_usuario[f"{user_id}_{circuito}_{numero}"] = 'esperando_opcao'
...
def tratar_causa_falha(message, circuito, numero):
    ...
    estado_resposta_usuario[f"{user_id}_{circuito}_{numero}"] = 'esperando_opcao'
    bot.send_message(message.chat.id, "*Qual a previsão de conclusão?* Favor informar no formato: *DD/MM/AA HH:MM*", parse_mode="Markdown")
...
def finaliza_informacao(message, chat_id, previsao_data, circuito, numero):
    ...
    if resposta == '1':
        ...
        try:
            ...
            estado_resposta_usuario[f"{chat_id}_{circuito}_{numero}"] = "cobrança_encerrada"
            salvar_estado_usuario(chat_id, '', circuito, numero),

            # Reiniciar a cobrança após a previsão
            respostas_gestor[circuito] = previsao_data
            estado_fluxo_concluido = False
            reparo = busca_nv_escalacao(circuito, numero)
            chat_id = message.chat.id
...
def tratar_previsao(message, circuito, numero, falha_ou_pendencia):
    global estado_resposta_usuario
    previsao = message.text.strip()
    user_id = message.chat.id

    # Regex para validar o formato DD/MM/AA HH:MM
    formato_data = r"\d{2}/\d{2}/\d{4} \d{2}:\d{2}"

    # Verifica se o formato é válido
    if not re.match(formato_data, previsao):
        bot.reply_to(message, "Formato de data inválido. Favor informar a data no formato: DD/MM/AA HH:MM")
        bot.register_next_step_handler(message, tratar_previsao, circuito, numero, falha_ou_pendencia)
        return
    ...
    try:
        ...
        if previsao_data <= datetime.now():
            bot.reply_to(message, "A data e hora de previsão deve ser maior que a data e hora atual. Informe uma nova data no formato: DD/MM/AA HH:MM")
            bot.register_next_step_handler(message, tratar_previsao, circuito, numero, falha_ou_pendencia)
            return
        ...
        bot.reply_to(message, f"Previsão de conclusão registrada: *{previsao}*", parse_mode="Markdown")
        bot.reply_to(message, "As informações repassadas foram registradas.\n*Gostaria de reenviar as informações?*\n1 - Sim.\n2 - Não. Até a próxima!", parse_mode="Markdown")
        bot.register_next_step_handler(message, finaliza_informacao, chat_id, previsao_data, circuito, numero)
        

    except ValueError:
        bot.reply_to(message, "Formato de data inválido. Tente novamente no formato: *DD/MM/AA HH:MM*", parse_mode="Markdown")
