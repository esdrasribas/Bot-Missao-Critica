...
from conn.mysqlConnector import atualizar_escalamento_inicial, buscar_reparos, atualizar_data_proxima_cobranca, buscar_chat_ids_todos, buscar_atualizacao, buscar_chatid_responsavel, buscar_reparos_escalamento, atualizar_escalamento_inicial_notificacao, obter_regional
import os
...
def notifica_reparos_criticos():
    ...
    if reparos_criticos:
        ...
        for reparo in reparos_criticos:
            ...
            ult_atualizacao = notificacao['data_cobranca'].strftime('%d/%m/%y %H:%M') if reparo.get('data_cobranca') else "_Sem atualizações anteriores._"
            ultima_mensagem = notificacao['ult_msg_crm'] if reparo.get('ult_msg_crm') else "_Sem mensagens anteriores._"
    ...
    if atualizar_reparos_escala:
        for reparo in atualizar_reparos_escala:
            ...
            if escala_inicial != nv_escalacao:
                ...
                logging.info(f"[atualiza_notificacao_escala] atualizar escalamento inicial notificacao")
                atualizar_escalamento_inicial_notificacao(nv_escalacao, escala_inicial, circuito, numero)
            else:
                logging.info(f"[atualiza_notificacao_escala] Sem atualizações na escala: {nv_escalacao}")
